...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.Fmoyennage = tk.DoubleVar(value=5.0)
        ...
        ctk.CTkSwitch(run_tab, text="Couper les pauses", variable=self.correction_temps).pack(pady=(20, 0))
            #derniers boutons
        ctk.CTkLabel(run_tab, text="Fréquence de moyennage (Hz)").pack(pady=(10, 0))
        ctk.CTkEntry(run_tab, textvariable=self.Fmoyennage).pack(pady=(10, 0))
        ...
        # onglet Logiciel 
        self.soft_tab = self.tabview.tab("Logiciel")
            # ajout du bouton dark mode
        self.appearance_mode_optionemenu = ctk.CTkOptionMenu(self.soft_tab, values=["Light", "Dark", "System"], command=self.change_appearance_mode_event)
        self.appearance_mode_optionemenu.pack(side=tk.BOTTOM, padx=20, pady=(10, 10))
        self.appearance_mode_label = ctk.CTkLabel(self.soft_tab, text="Appearance Mode:", anchor="w")
        self.appearance_mode_label.pack(side=tk.BOTTOM, padx=20, pady=(10, 0))
    ...
    def analyze_data(self):
        ...
        Fmoyennage = self.Fmoyennage.get()
        ...
        def moyennage(t, A, F=1):
            Am = []
            freq = int(1/F*len(t)/t[-1])
            i, j = 0, freq
            while j < len(t):
                Am.append(np.mean(A[i:j]))
                i, j = j, j+freq
            tm = np.linspace(0, t[-1], len(Am))
            return tm, Am
        ...
        if correction_temps:
            ...
            tm, Accm = moyennage(t, Acc, Fmoyennage)
        ...
        tm, Accm = moyennage(t, Acc, Fmoyennage) # ax8 : accélération
